...
// Replace from line 1241 (index 1240) to line 1553 (index 1552)
const lines = content.split('\n');

// Find where classicWords ends before line 1241
// Line 1241 starts with a broken entry.
lines[1240] = '  { word: "دەرپێ", hint: "ئەو جلکێ ناڤخۆیی یێ کو د بن پانتەڕۆنی یان شەلوالیدا دهێتە پۆشین.", category: "جلوبەرگ" }';
lines[1241] = '];';
lines[1242] = '';
lines[1243] = 'export const hardWords = [';
lines[1244] = '  ...recoveredFromDistRaw.filter(w => w.word.length >= 6),';
lines[1245] = '  ...humanNamesRaw.filter(n => n.word.length >= 6),';
lines[1246] = '  ...adjectivesWordsRaw.filter(a => a.word.length >= 6),';
lines[1247] = '];';
lines[1248] = '';
lines[1249] = 'export const mamakWords = ' + mamakWordsJS + ';';

// Now remove lines from 1250 until line 1553
// Wait, I need to know the index of `export const gameWordLists = {`
let gameWordListsIndex = lines.findIndex(l => l.includes('export const gameWordLists = {'));
if (gameWordListsIndex === -1) {
  console.error('Could not find gameWordLists');
  process.exit(1);
}

// Remove lines between 1250 and gameWordListsIndex
lines.splice(1250, gameWordListsIndex - 1250);
