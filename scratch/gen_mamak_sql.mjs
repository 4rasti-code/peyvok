...
let sql = '-- Migration: Standardize Mamak (Riddles) Category\n';
sql += '-- Description: Consolidates all riddles into the "مامک" category and updates hints.\n\n';
sql += '-- 1. Update existing words that might be incorrectly categorized\n';
sql += 'UPDATE words SET category = \'مامک\' WHERE word IN (\n';
sql += mamakWords.map(w => `  '${w.word.replace(/'/g, "''")}'`).join(',\n');
sql += '\n);\n\n';

sql += '-- 2. Insert or Update all Mamak words\n';
sql += 'INSERT INTO words (word, hint, category)\nVALUES\n';
sql += mamakWords.map(w => `  ('${w.word.replace(/'/g, "''")}', '${w.hint.replace(/'/g, "''")}', 'مامک')`).join(',\n');
sql += '\nON CONFLICT (word) DO UPDATE SET\n';
sql += '  hint = EXCLUDED.hint,\n';
sql += '  category = EXCLUDED.category;\n\n';

sql += '-- 3. Remove words from other categories that are now Mamaks (to avoid semantic duplicates if any)\n';
// Actually ON CONFLICT handles this if word is primary key.

fs.writeFileSync('d:/Peyivcin App/sync_mamak_standardization.sql', sql, 'utf8');
console.log('Successfully generated sync_mamak_standardization.sql');
