-- Append Retention and Fever Highscore stats to profiles
-- Safely adds new columns and updates the sync RPC
...
-- Update RPC function to handle retention and fever highscore
CREATE OR REPLACE FUNCTION sync_game_stats(
    ...
    p_fever_score INTEGER DEFAULT 0
) RETURNS VOID AS $$
DECLARE
    v_user_id UUID;
...
BEGIN
    ...
    -- 2. Increment games_played and mode_play_counts
    UPDATE profiles 
    SET 
    ...
    -- 3. Word Fever Highscore Logic
    IF p_mode = 'word_fever' AND p_fever_score > 0 THEN
        UPDATE profiles 
        SET fever_highscore = GREATEST(COALESCE(fever_highscore, 0), p_fever_score)
        WHERE id = v_user_id;
    END IF;
